...
import { createMockDocument, createMockCancellationToken } from './mocks/vscode'
...
describe('MochaCodeLensProvider', () => {
    ...
    const getCodeLenses = (
        document: mockVscode.TextDocument
    ): mockVscode.CodeLens[] => {
        ...
        return (Array.isArray(result)
            ? result
            : []) as unknown as mockVscode.CodeLens[]
    }
  ...
  it('should work', () => {});
        ...
        it('should create Debug Test command with correct arguments', () => {
            ...
            const debugLens = lenses.find(
                (l) => l.command?.command === 'mochaTestLens.debugTest'
            )

            assert.ok(debugLens)
            assert.strictEqual(
                debugLens.command?.title,
                '$(debug-alt) Debug Test'
            )
...
describe('Second', () => {});`,
  ...
  it('should work', () => {});
            ...
            const itRunLens = lenses.find(
                (l) =>
                    l.command?.command === 'mochaTestLens.runTest' &&
                    l.command?.tooltip?.includes('should work')
            )

            assert.ok(itRunLens)
            assert.strictEqual(
                itRunLens.command?.tooltip,
                'Run: MyClass should work'
            )
        })
  ...
  describe('Inner', () => {
    ...
    describe('onDidChangeCodeLenses', () => {
        it('should be an event that can be subscribed to', () => {
            ...
            assert.strictEqual(
                typeof provider.onDidChangeCodeLenses,
                'function'
            )
        })
    })
